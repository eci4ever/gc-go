...
export function hasOrganizationPermission(
  organization: Pick<OrganizationMembership, "role"> &
    Partial<Pick<OrganizationMembership, "permissions">>,
  permission: string,
) {
  ...
  return ["members.read", "teams.read"].includes(permission);
}
...
export function organizationQueryOptions(slug: string) {
  return queryOptions({
    ...
    queryFn: async () => {
      const result = await request<{
        organization: Omit<OrganizationMembership, "permissions">;
        permissions: string[];
...
export function organizationRolesQueryOptions(slug: string) {
  return queryOptions({
    queryKey: ["organizations", slug, "roles"] as const,
    queryFn: async () => {
      const result = await request<{
        permissions: OrganizationPermission[];
        roles: OrganizationCustomRole[];
      }>(`/api/organizations/${encodeURIComponent(slug)}/roles`);
      return {
        permissions: result.permissions ?? [],
        roles: result.roles ?? [],
      };
...
export function createOrganizationRole(
  ...
  return request<{ role: OrganizationCustomRole; permissions: string[] }>(
    `/api/organizations/${encodeURIComponent(slug)}/roles`,
...
export function updateOrganizationRole(
  ...
  return request<{ role: OrganizationCustomRole; permissions: string[] }>(
    `/api/organizations/${encodeURIComponent(slug)}/roles/${encodeURIComponent(roleId)}`,
    { method: "PUT", body: JSON.stringify(input) },
  );
}

export function deleteOrganizationRole(slug: string, roleId: string) {
  return emptyRequest(
    `/api/organizations/${encodeURIComponent(slug)}/roles/${encodeURIComponent(roleId)}`,
    { method: "DELETE" },
  );
