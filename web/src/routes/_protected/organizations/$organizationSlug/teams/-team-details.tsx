...
export function TeamDetails() {
  ...
  const lifecycle = useMutation({
    mutationFn: async (action: Exclude<Confirmation, null>) => {
      if (action === "delete")
        return deleteOrganizationTeam(organizationSlug, teamId);
      return archiveOrganizationTeam(
        organizationSlug,
        teamId,
        action === "archive",
      );
  ...
  const invite = useMutation({
    mutationFn: () =>
      inviteOrganizationMember(organizationSlug, {
        email: inviteEmail,
        role: inviteRole,
        teamId,
      }),
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2 font-medium">
              {row.original.name}
              {team?.leadUserId === row.original.userId && (
      ...
      {canManage && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {archived
                ? "Restore this team to edit its details."
        ...
        <CardContent className="space-y-4">
          {membersEditable && selectedAssigned.length > 0 && (
            <Button
              ...
              disabled={bulk.isPending}
              onClick={() =>
                bulk.mutate({ action: "remove", userIds: selectedAssigned })
              }
            >
      ...
      {membersEditable && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Select active organization members to add to this team.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {selectedAvailable.length > 0 && (
              <Button
                disabled={bulk.isPending}
                onClick={() =>
                  bulk.mutate({ action: "add", userIds: selectedAvailable })
                }
              >
